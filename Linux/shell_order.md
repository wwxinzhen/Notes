...
.tar.gz
	tar -zxf demo.tar.gz -C 	//解压demo.tar.gz到当前目录
	tar -cvf test.tar.gz test	//将test文件夹打包
```
...
tail [agreement] [file]
	tail -f file1			//循环读取		
	tail -q file1			//不显示处理信息
	tail -n 3 file1			//查看文件最后三行
	tail -n -100 file1		//除了前99行不显示外，显示第100行到末尾行
```
...
net stop lexxmanager        //关闭环境变量
net start lexxmanager       //启动环境变量
chown -R [用户名] [文件名]    //更换文件所属用户（ubuntu）
chgrp -R [用户名] [文件名]    //更换文件所属用户组（ubuntu）
=========跨服务器拷贝文件===========
scp [绝对路径]  username@ip:/[绝对路径]  	//将本地文件拷贝到远端
scp -r [绝对路径]  username@ip:/[绝对路径]  //将本地文件夹拷贝到远端
scp username@ip:/[绝对路径] [绝对路径]      //将远端文件拷贝到本地
scp -r username@ip:/[绝对路径] [绝对路径]	//将远端文件夹拷贝到本地
==================================
tail -f filename             //把文件最尾部的内容显示在屏幕上，并不断刷新
==================================
vi复制一行
	把光标移到要复制的行上
	按 yy
...
netstat -ant | grep 8080 | wc -l						//统计8080端口有多少个TCP连接
netstat -ant | grep 8080 | grep ESTABLISHED | wc -l 	//TCP连接中有多少连接状态是ESTABLISHED
